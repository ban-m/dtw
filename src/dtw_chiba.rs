...
fn nextpos<D,F,T>(x1:&[D],
                  ...
                  x1pos:usize,
                  x2pos:usize,
                  dist:&F,band:usize)->(usize,usize)
    where F:Fn(&D,&D) -> T,T:Float
{
    let current_score = dp[matrix_to_dp_idx(x1pos,x2pos,band)]
        - dist(&x1[x1pos-1],&x2[x2pos-1]);
...
fn detect_err_about_band(x1len:usize,x2len:usize,band:usize) -> Result<(),String>{
    match band {
        ...
        n if n > x1len || n > x2len => Err("Band width should be smaller than query length".to_string()),
...
pub fn dtw_chiba<D,F,T>(x1:&[D],x2:&[D],dist:&F,band:usize)->
