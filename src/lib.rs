...
/// Return type of DTW functions.
pub type TRACE = (Vec<usize>, Vec<usize>);
...
impl std::fmt::Display for Mode {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        let name = match *self {
            ...
            Mode::Scouting(num_scouts, num_packs) => {
                format!("Scouting({}_{})", num_scouts, num_packs)
            }
        };
        write!(f, "{}", name)
    }
...
pub fn dtw<D, F, T>(x1: &[D], x2: &[D], mode: Mode, dist: &F) -> Result<(T, TRACE, usize), String>
where
    F: Fn(&D, &D) -> T,
    T: Float,
    D: Num + Copy,
{
    let err = format!(
        "invalid input: the lengths are different,while restricted mode was chosen:{},{}",
        x1.len(),
        x2.len()
    );
    match mode {
        ...
        Mode::Scouting(num_scouts, num_packs) => {
            scoutingdtw::scouting_dtw(x1, x2, dist, Some(num_scouts), Some(num_packs))
        }
    }
}

fn dtw_norestrict<D, F, T>(
    x1: &[D],
    x2: &[D],
    dist: &F,
    mode: Mode,
    ...
    T: Float,
{
    ...
    let mut res = match mode {
        ...
        Mode::Sub => {
            let mut dp = vec![vec![T::infinity(); x2len + 1]; x1len + 1];
            for j in 0..x2len {
                dp[0][j] = num::zero();
            }
            dp
    ...
    let (mut x2pos, score) =
        match mode {
            ...
            Mode::Sub => res[x1len].iter().enumerate().fold(
                (0, Float::infinity()),
                |(idx, acc), (i, &score)| if acc < score { (idx, acc) } else { (i, score) },
            ),
            _ => unreachable!(),
        };
    while x1pos > 0 {
    ...
    T: Float,
{
    ...
    for item in dp.iter() {
        ...
        for j in 0..column {
            ...
            if item[j].is_infinite() {
                print!("inf");
            } else {
                //print!("{:3.0}",dp[i][j].to_f32().unwrap());
                print!("{:3.0}", item[j].to_f32().unwrap());
            }
        }
        println!();
    }
