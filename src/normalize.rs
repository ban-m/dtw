...
fn z_normalize(xs:&[f32])->Vec<f32>{
    ...
    // }
    let res = xs.iter().map(|&x|((x as f64 -mean)/stdev) as f32).collect::<Vec<_>>();
    res
}
#[test]
fn test(){
...
fn max_min_normalize(xs:&[f32])->Vec<f32>{
    let (max,min) = xs.iter()
        .fold((f32::MIN,f32::MAX),|(big,small),&x| (big.max(x),small.min(x)) );
    if max == min {
...
fn max_min_normalize_mut(xs:&mut [f32]){
    let max = xs.iter().fold(-1000000.,|max,&x| if max < x { x } else { max });
    let min = xs.iter().fold( 1000000.,|min,&x| if min < x { min } else { x });
    for x in xs.iter_mut(){
        *x = (*x - min ) / max ;
    }
}
...
pub fn histgram_modify(xs:&[f32],cdf:&[f32]) -> Vec<f32>{
    ...
    for k in 0..n{
        ...
        if i >= size-1 {
            ...
            res[k] = (cdf[i+1]-cdf[i])*range/DX * ( xs[k] - LOWER - i as f32 *DX) + cdf[i]*range + LOWER;
        }
    }
    res
}
